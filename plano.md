...
* Como não há ferramentas automatizadas de editor, use o usuário como ponte: para "despachar" agentes, gere o Envelope de Despacho em texto e peça para o usuário copiá-lo para um novo chat. Para ler arquivos, pergunte ao usuário.
* Todo estado é armazenado em arquivos Markdown em `data/` — leia e escreva esses arquivos diretamente (imprimindo-os no chat).
...
#### Envelope de Despacho (Maestro constrói este prompt e pede para o usuário copiar para um novo chat)
...
#### Envelope de Resposta (agente despachado retorna isto para o usuário colar de volta no Maestro)
...
- **Orquestração**: Roda no chat usando o usuário como "roteador" para simular o `spawn_agent` e fazer o despacho de sub-agentes copiando e colando entre as abas do chat.
